...
import { validateClaim } from '../supabase/functions/_shared/payments/claimPolicy.ts';
...
// ─── 수령 정책: 받기/기부하기 2택 ────────────────────────
const claimOrder = { recipient_id: 'user-b', status: 'paid' };

test('수령 정책 — 받는 사람 본인만 받기/기부 가능', () => {
  assert.deepEqual(validateClaim(claimOrder, 'user-b', 'receive'), { ok: true });
  assert.deepEqual(validateClaim(claimOrder, 'user-b', 'donate'), { ok: true });
  assert.deepEqual(validateClaim(claimOrder, 'user-a', 'receive'), { ok: false, reason: 'not_recipient' });
});

test('수령 정책 — paid 상태에서만 선택 가능 (중복 수령·결제 전 수령 차단)', () => {
  for (const status of ['created', 'issued', 'delivered', 'donated', 'auto_refund']) {
    const v = validateClaim({ ...claimOrder, status }, 'user-b', 'receive');
    assert.deepEqual(v, { ok: false, reason: 'not_claimable' }, `status=${status}`);
  }
});

test('수령 정책 — 정의되지 않은 action 거부', () => {
  // @ts-expect-error 잘못된 action 런타임 방어 확인
  const v = validateClaim(claimOrder, 'user-b', 'steal');
  assert.deepEqual(v, { ok: false, reason: 'invalid_action' });
});
