// 🚀 완주 매칭 기부(0075) 표시용 순수 로직 — 완주자 집계(countCompleters) + 금액 표시(formatWon).
// 이 숫자가 곧 조직이 실제로 낼 기부액("완주자 1명당 N원")이라 틀리면 잘못된 금액을 공개하게 된다.
// 완주 판정 자체는 goalStatus(단일 소스)가 하고, 여기서 검증하는 건 "누구를 세느냐"다.
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { countCompleters } from '../mobile/lib/stats.ts';
...
test('완주한 도전자만 센다', () => {
  ...
  assert.equal(countCompleters(CH, members, proofs), 1);
});

test('주최자(role=host)는 완주해도 세지 않는다 — 도전자가 아니다 (0069)', () => {
...
test('아무도 없으면 0', () => {
  assert.equal(countCompleters(CH, [], []), 0);
});
...
test('count 유형 — 목표 개수를 채운 사람 (종료 무관·조기 완주)', () => {
  ...
  assert.equal(countCompleters(countCh, members, proofs), 1);
});
