...
| 숨은 탭 | `discover`(둘러보기)·`profile`(내 정보) = `href:null` (탭바에 없음, 직접 라우트·아바타로만 진입) |
...
#### [discover.tsx](mobile/app/(tabs)/discover.tsx) — 둘러보기 · 539줄 · **탭바 숨김(`href:null`)**
- (a) 공개(open) 하다 탐색 + 4가지 평가(✨😱🥹💫). MVP 큐레이션 = "지금 핫한"(최근순) 단일.
- (b) `AppHeader` + 서브헤더 + 큐레이션 안내 + **카테고리 필터 칩(가로 스크롤)** + FlatList. `DiscCard` = 사회공헌 뱃지·D-N + 제목·설명 + 인원·기간 + 진행률 바 + 사회공헌 배너 + 4평가 칩 + 푸터(@크리에이터·카테고리·+하다/+동참).
- (c) 진입: **탭에 없음** — 코드상 직접 라우트 접근만 가능(현재 UI 진입점 미연결). 이탈: 카드 → `/room/[id]`.
...
- 숨은 라우트: `discover`·`profile` = `href:null` (탭바 미노출, 직접 라우트/아바타 진입).
...
### 3.2 둘러보기/공개 챌린지 → 참여
- **둘러보기(discover)**: 현재 탭바에 노출 안 됨(`href:null`) — 코드상 동선 단절. 진입 시 카드 → `/room/[id]`(비멤버 미리보기).
...
| discover | Skeleton ×3 | ErrorState | 🌍 "공개 하다 없어요"(필터 시 분류별 문구) |
...
| 둘러보기 탭 | "하단 5탭… 둘러보기" 전제(§3.8) | `discover` = `href:null` (탭바 미노출) | 부분구현(동선 단절) |
| 둘러보기 큐레이션 3종 | 지금 핫한/신규/인기 | **"지금 핫한"(최근순) 단일**만 | 부분구현 |
...
### 8.5 둘러보기 ([discover.tsx](mobile/app/(tabs)/discover.tsx)) · 탭바 숨김
```
┌─ AppHeader ──────────────────────────────┐
│ 둘러보기                                   │
│ 🔥 지금 핫한 · 최근 만들어진 공개 하다      │
...
- **동선 단절(중요)**: 둘러보기(discover)는 탭에서 빠진 정도가 아니라 **앱 전체에 진입점이 0개**(`router.push('/discover')`·href 어디에도 없음). 539줄 완성 화면이 통째로 고아 상태. 게다가 **챌린지 4가지 평가(✨😱🥹💫) UI가 discover에만 존재** → 이 화면을 숨기면서 평가 기능 전체가 사용자에게 비노출(CLAUDE.md 수칙 #8 "4가지 평가 의미 보존"과 충돌). 홈 "함께 합류하기"는 단순 합류 카드라 평가 UI 없음.
